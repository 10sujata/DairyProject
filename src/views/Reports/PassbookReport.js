...
class PassbookReport extends Component {
  ...
  btnNextClick() {
    ...
    exportCsv(){
      var csvRow = [];
      var newarray = [['S.No.','Center No.','Society No.','Farmer No.','Date','Shift','Cattle Type','Quantity','Amount']];
      var re = this.state.filteredUsers;
      for(var item=0;item<re.length;item++){
        newarray.push([item,re[item].centernumber,re[item].rmcunumber,re[item].farmer_number,re[item].created_at,re[item].shift,re[item].cattletype,re[item].sum_of_totalweight,re[item].sum_of_totalamount]);
      }
    ...
    if(isNextBtnActive === 'disabled') {
     ...
     return(
        ...
        <Card>
              ...
              <CardHeader>
                <i className="fa fa-align-justify"></i> Member Passbook for Period &nbsp;
