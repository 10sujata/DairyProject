...
class CallsToAttend extends Component {
    ...
    constructor(props) {
      ...
      onOpenModal = () => {
        ...
        // if(this.state.customername==""||this.state.product_category==""||this.state.startdate==""){
        //   alert("please select atleast one filter");
        // }
         if(this.state.customername!="" && this.state.product_category!="") {
          const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("productcategory")
              userref.on("value", snapshot =>{
                console.log("dsdsds",snapshot.val());
                let allUsers = snapshot.val();
                let newState = [];
                for(let user in allUsers){
                    if(allUsers[user].user_uid==isLoggedIn && allUsers[user].customer_name==this.state.customername && allUsers[user].status_of_complaint=="empty"){
                      newState.push({
                        id: user,
        ...
        console.log("dcustomer name",this.state.customername);
        const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("customer_name").equalTo(this.state.customername)
            userref.on("value", snapshot =>{
              ...
              let newState = [];
              for(let user in allUsers){
                  if(allUsers[user].user_uid==isLoggedIn && allUsers[user].status_of_complaint=="empty"){
                    newState.push({
                      id: user,
                      Name: allUsers[user].name_of_service_engineer,
                      Region: allUsers[user].region_of_service_engineer,
      ...
      console.log("dcustomer name",this.state.product_category);
  ...
  const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("call_log_date").startAt(this.state.startdate).endAt(this.state.enddate)
      userref.on("value", snapshot =>{
        ...
        let newState = [];
        for(let user in allUsers){
            if(allUsers[user].user_uid==isLoggedIn && allUsers[user].status_of_complaint=="empty" && allUsers[user].productcategory==this.state.product_category && allUsers[user].customer_name==this.state.customername){
              newState.push({
                id: user,
                Name: allUsers[user].name_of_service_engineer,
                Region: allUsers[user].region_of_service_engineer,
      ...
      const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("call_log_date").startAt(this.state.startdate).endAt(this.state.enddate)
          userref.on("value", snapshot =>{
            ...
            let newState = [];
            for(let user in allUsers){
                if(allUsers[user].user_uid==isLoggedIn && allUsers[user].status_of_complaint=="empty" && allUsers[user].product_category==this.state.product_category){
                  newState.push({
                    id: user,
                    Name: allUsers[user].name_of_service_engineer,
                    Region: allUsers[user].region_of_service_engineer,
      ...
      const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("call_log_date").startAt(this.state.startdate).endAt(this.state.enddate)
          userref.on("value", snapshot =>{
            ...
            let newState = [];
            for(let user in allUsers){
                if(allUsers[user].user_uid==isLoggedIn && allUsers[user].status_of_complaint=="empty" && allUsers[user].customer_name==this.state.customername){
                  newState.push({
                    id: user,
                    Name: allUsers[user].name_of_service_engineer,
                    Region: allUsers[user].region_of_service_engineer,
  ...
  btnNextClick() {
      ...
      globalSearch = () => {
        ...
        let filteredData = this.state.callsAttend.filter(value => {
          //console.log("",value);
          return (
            value.customer_name.toLowerCase().includes(searchInput.toLowerCase()) ||
            value.customer_state.toLowerCase().includes(searchInput.toLowerCase()) ||
    ...
    componentDidMount() {
      const isLoggedIn = localStorage.getItem("appState");
      const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("user_uid").equalTo(isLoggedIn);
          userref.on("value", snapshot =>{
            let allUsers = snapshot.val();
            console.log("get all users in a list",allUsers);
            let newState = [];
            for(let user in allUsers){
              if(allUsers[user].status_of_complaint=="empty"){
                newState.push({
                  id: user,
                  region:allUsers[user].region_of_service_engineer,
            ...
            this.setState({
              callsAttend: newState,filteredUsers: newState,
            });
    ...
    exportCsv(){
      var csvRow = [];
      var allUsers1 = [['Region','Service Engineer Name','Customer Name','Customer Email','Customer Country','Customer State','Customer City','Call Assign By','Call Assign To ','Call Log Date','Call Visit Date','City Of Service Call','Product Category','Product Description','Service Call Status']];
      var re = this.state.allUsers1;
      this.state.allUsers1.forEach((user) => {        
          let userArray = [user.Region,user.Name,user.Customername,user.CustomerEmail,user.CustomerCountry,user.CustomerState,user.CustomerCity,user.Callassignby,user.Callassignto,user.Calllogdate,user.Callvisitdate,user.Cityofservicecall,user.Productca,user.Productdes,user.Servicecallstatus];
     ...
     const wsAll = XLSX.utils.aoa_to_sheet(allUsers1)
         ...
         XLSX.writeFile(wb, "callstoattend.xlsx")
    ...
    handleSetData = callsAttend => {
      this.setState({ filteredUsers: callsAttend });
    };
    ...
    render() {
      ...
      let { allUsers1,filteredUsers,currentPage, todosPerPage,upperPageBound,lowerPageBound,isPrevBtnActive,isNextBtnActive,open } = this.state;
      const indexOfLastTodo = currentPage * todosPerPage;
      const indexOfFirstTodo = indexOfLastTodo - todosPerPage;
      console.log("==",this.state.allUsers1);
      ...
      let currentTodos;
       ...
       const renderTodos = currentTodos.map((partners, index) => {
        ...
        <td>
        <Button size="sm"  block color="primary" onClick={()=>this.routeChange(partners.id)}>
         Call Attend
        </Button>
        </td>
    </tr>
        ...
        <Card>
              <CardHeader>
                <div className="searchnewclass1">
                  <i className="fa fa-align-justify"></i> Calls To Attend
