...
class CompltedCalls extends Component {
    ...
    constructor(props) {
               ...
               const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("product_category").equalTo(this.state.product_category)
                   userref.on("value", snapshot =>{
                     ...
                     for(let user in allUsers){
                         if(allUsers[user].customer_name==this.state.customername && allUsers[user].status_of_complaint=="Completed"){
                           newState.push({
                             id: user,
      ...
      const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("call_log_date").startAt(this.state.startdate).endAt(this.state.enddate)
          userref.on("value", snapshot =>{
            ...
            let newState = [];
            for(let user in allUsers){
                if(allUsers[user].status_of_complaint=="Completed" && allUsers[user].product_category==this.state.product_category && allUsers[user].customer_name==this.state.customername){
                  newState.push({
                    id: user,
                    Name: allUsers[user].name_of_service_engineer,
                    Region: allUsers[user].region_of_service_engineer,
        ...
        const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("call_log_date").startAt(this.state.startdate).endAt(this.state.enddate)
            userref.on("value", snapshot =>{
              ...
              let newState = [];
              for(let user in allUsers){
                  if(allUsers[user].status_of_complaint=="Completed" && allUsers[user].product_category===this.state.product_category){
                    newState.push({
                      id: user,
                      Name: allUsers[user].name_of_service_engineer,
                      Region: allUsers[user].region_of_service_engineer,
    ...
    const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("call_log_date").startAt(this.state.startdate).endAt(this.state.enddate)
        userref.on("value", snapshot =>{
          ...
          let newState = [];
          for(let user in allUsers){
              if(allUsers[user].status_of_complaint=="Completed" && allUsers[user].customer_name==this.state.customername){
                newState.push({
                  id: user,
                  Name: allUsers[user].name_of_service_engineer,
                  Region: allUsers[user].region_of_service_engineer,
  ...
  btnNextClick() {
    ...
    componentDidMount() {
      const userref =  firebaseConfig.database().ref(`/Calls/`).orderByChild("status_of_complaint").equalTo("Completed");
    ...
    exportCsv(){
     ...
     const wsAll = XLSX.utils.aoa_to_sheet(allUsers1)
         ...
         XLSX.writeFile(wb, "completedcalls.xlsx")
    ...
    viewcompletedcall(id) {
      let path = `/viewcompletedcalls/`+id;
      this.props.history.push(path);
    }
    ...
    createpdf(id) {
      ...
      userref.on("value", snapshot => {
        ...
        const MyDoc = ({data}) => (
          ...
          <Document>
            <Page>
            <Text>{data.name_of_service_engineer}</Text>
            </Page>
          </Document>
    ...
    render() {
      ...
      let currentTodos;
       ...
       const renderTodos = currentTodos.map((partners, index) => {
        ...
        <td>
          <span size="md" id="grid" onClick={()=>this.viewcompletedcall(partners.id)}>
          <i className="fa fa-eye fa-lg float-center"></i> &nbsp;&nbsp;
          </span>&nbsp;
          <span size="md" id="grid" onClick={()=>this.createpdf(partners.id)}>
          <i className="fa fa-download fa-lg float-center"></i> &nbsp;&nbsp;
          </span>&nbsp;
        ...
        <Card>
              <CardHeader>
                <div className="searchnewclass1">
                  <i className="fa fa-align-justify"></i> Completed Calls
