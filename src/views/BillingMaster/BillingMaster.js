...
class BillingMaster extends Component {
    ...
    this.setState({ 
      ...
      allrmcucenterdata:cdss,  
      allfarmernamedata:farmernamearray,
    ...
    setTimeout(() => {
      ...
      onOpenModal = () => {
        axios.post(API_BASE_URL+'/billinglistid',{
          ...
          farmernumber:this.state.farmernumber,         
               ...
               if(((response.data.data).length)>0){
                ...
                axios.post(API_BASE_URL+'/farmercollectiondata1',{
                  farmernumber: this.state.farmernumber,
                  fromdate: this.state.fromdate,
                  todate:this.state.todate
                  }).then(response => {      
                    console.log("cfsdfdfsfsfdsfsfdsf",response);    
                    let allcattletype = response.data.data;          
                    let cattlevalue = '';
                    let farmercenternumber = '';
                    for(let user in allcattletype){
                      cattlevalue = allcattletype[user].cattletype
                      farmercenternumber = allcattletype[user].centernumber
                    }
                 ...
                 return prev + +current.lacto
                   ...
                   this.setState({
                     ...
                     farmercenternumber:farmercenternumber,
              ...
              axios.post(API_BASE_URL+'/farmerdeductiondata',{
                farmernumber: this.state.farmernumber,
                fromdate: this.state.fromdate,
                todate:this.state.todate
                }).then(response => {      
    ...
    settingvalue(e) {
      ...
      submitData(e) {
        ...
        axios.post(API_BASE_URL+'/billform',{
         ...
         fromdate:this.state.fromdate,
         todate:this.state.todate,
         centercode:this.state.farmercenternumber,
         farmername:this.state.farmername1,
         milkpour:this.state.totalneweight,
         milkamount:this.state.totalamount,
         deductionamount:this.state.deductionamounttotal,
         totalamount:paypabalamount
      })
              ...
              axios.get(API_BASE_URL+'/centerfarmer/' + centerid)
                .then(response => {
                  ...
                  for(let k=0; k<dataarray2.length;k++){
                    if(dataarray2[j].id!=""){
                      const farmerid = dataarray2[k].farmer_number;                                           
                         axios.post(API_BASE_URL+'/farmercollectiondatasubmit',{
                            farmerid:farmerid,                              
                            fromdate:this.state.fromdate,
                            todate:this.state.todate,                            
                          }).then(response => {
                            axios.post(API_BASE_URL+'/newsubmitform',{
                              data:response.data.data[0],  
                              date:date,
                              fromdate:this.state.fromdate,
                              todate:this.state.todate,                            
                            }).then(response => {
                                console.log("cddfdsfdsfs");
                          })
                        })                       
                    }
                  }
    ...
    render() {
      const { errors,allfarmernamedata,allrmcucenterdata,allrmcudata,allcenternamearray,allfarmercollectionnewdata,allfarmercollectionnewdata1,allfarmercollectionnewdata2,farmercdata,billnumber1,open1,deductiondata,farmernamenew,centernamenew,open,filteredUsers,opennew,alltransaction,allother,collectiondata } = this.state;
    this.state.billnumber = "000"+(Number(this.state.billnumber1)+1);
    const merge3 = allrmcucenterdata.flat();
    
    const merge4 = allfarmernamedata.flat();
     ...
     return(
        ...
        <Container className="collectionconatiner">
          <span className="error">{this.state.errors["billerror"]}</span>
        ...
        <Form onSubmit={this.submitData}>
          ...
          <Col lg="12" md="12">
            ...
            {allrmcudata.map(outerElement => (
              merge3.map(innerElement => (               
                <div>
                <Row gutter={20}> 
                <Col lg="12" md="12">
                  <div className="billdiv">                    
                      <h5 className="modal-title" align="center">{outerElement.society_name}/{innerElement.center_name}</h5>                                       
                  </div>
                </Col><br />
                <Col lg="6">
                ...
                <Row gutter={20}>  
                            <Col lg="6">
                              <div className="date-area">
                                ...
                                <div className="date-area-right">                                                         
                                <Label htmlFor="chartname">{outerElement.society_name}-{outerElement.society_number}</Label>
                                </div>
                              </div>
                            </Col>
                            ...
                            <div className="date-area">
                              ...
                              <div className="date-area-right">
                                  <Label htmlFor="chartname">{innerElement.center_name}-{innerElement.center_number}</Label>
                              </div>
                          ...
                          <Table responsive bordered>
                              ...
                              <tr>
                                ...
                                {merge4.map((content) =>{                                   
                                  return (
                                  content.centernumber === innerElement.center_number ? ( 
                                    <tbody>                                                                        
                                      {arr3.map((item) =>{
                                      ...
                                      return (
                                      item.farmernumber === content.farmer_number ? ( 
                                        <tr>
                                        <td>{content.farmer_number}</td>
                                        <td>{content.farmername}</td>                                  
            ...
            <Button type="button" size="md" color="primary" onClick={this.onSubmitData}><i className="fa fa-dot-circle-o"></i> Submit</Button>&nbsp;&nbsp;
            </Form>  
