...
class SupplierList extends Component {
    ...
    addproduct(){
      let path = `/listsupplier/suppliermaster/`;
      this.props.history.push(path);
    }
  ...
  btnNextClick() {
      ...
      globalSearch = () => {
        ...
        let filteredData = this.state.allUsers.filter(value => {
          //console.log("",value);
          return (
            value.supplier_name.toLowerCase().includes(searchInput.toLowerCase()) ||
            value.supplier_address.toLowerCase().includes(searchInput.toLowerCase())            
          );
        });
      ...
      editsupplier(id) {
        let path = `/editsupplier/`+id;
        this.props.history.push(path);
      }

      handleClickDelete = (id) => {
          axios.post(API_BASE_URL+'/deletesupplier/' + id)
          .then(response => {
    ...
    exportCsv(){
      var csvRow = [];
      var newarray = [['id','Supplier Name','Supplier Name In Regional Language','Record Name','Address','Supplier Number','Owner Name']];
      var re = this.state.filteredUsers;
      for(var item=0;item<re.length;item++){
        newarray.push([item,re[item].supplier_name,re[item].supplier_name1,re[item].record_list,re[item].supplier_address,re[item].supplier_number,re[item].ower_name]);
      }
    ...
    render() {
      ...
      const renderTodos = currentTodos.map((partners, index) => {
        ...
        <td>
            {partners.supplier_name}
        </td>
        <td>{partners.supplier_name1}</td>
        <td>{partners.supplier_address}</td>
        <td>{partners.supplier_number}</td>
        <td>
        <span size="md" id="grid" onClick={()=>this.deletesupplier(partners.id)}>
        ...
        <td>
        <span size="md" id="grid" onClick={()=>this.editsupplier(partners.id)}>
     ...
     return(
        ...
        <Card>
              ...
              <CardHeader>
                ...
                <Button size="lg" onClick={()=>{this.addproduct()}}>Add Supplier</Button>&nbsp;
  <Input id="input-search" data={this.state.filteredUsers} onChange={this.handleChange} handleSetData={this.handleSetData} type = "text" placeholder = "Search Supplier" value={this.state.searchInput || ""} name = "search" id = "search"/>&nbsp;
              </CardHeader>
              <CardBody>
                <Table responsive bordered>
                  ...
                  <tr>
                    <th>Supplier Name</th>
                    <th>Supplier Name In Regional Language *</th>
