...
class DeviceList extends Component {
    ...
    adddevice(){
      let path = `/listdevice/devicemaster/`;
      this.props.history.push(path);
    }
  ...
  btnNextClick() {
    ...
    componentDidMount() {
      ...
      axios.get(API_BASE_URL+'/listdevices')
      .then(response => {
    ...
    handleClickDelete = (id) => {
        axios.post(API_BASE_URL+'/deletedevice/' + id)
        .then(response => {
   ...
   deleteDevice = (id) => {
    ...
    render() {
      ...
      const renderTodos = currentTodos.map((partners, index) => {
        ...
        <td>
            {partners.device_name1}
        </td>
        <td>{partners.device_name2}</td>
        <td>{partners.no_devices}</td>

        <td>

        <i className="fa fa-trash fa-lg float-center" onClick={()=>this.deleteDevice(partners.id)}></i>&nbsp;&nbsp;&nbsp;
        <i className="fa fa-edit fa-lg float-center" onClick={()=>this.editdevice(partners.id)}></i>
  ...
  {/* <Input id="input-search" data={this.state.filteredUsers} onChange={this.handleChange} handleSetData={this.handleSetData} type = "text" placeholder = "Search Engineer" value={this.state.searchInput || ""} name = "search" id = "search"/>&nbsp; */}
              ...
              <CardBody>
                <Table responsive bordered>
                  ...
                  <tr>
                    <th>Device Name</th>
                    <th>Device Name in Regional Language</th>
