...
class SocietyList extends Component {
  ...
  btnNextClick() {
      ...
      globalSearch = () => {
        ...
        let filteredData = this.state.allUsers.filter(value => {
          console.log("",value);
          return (
            value.society_name.toLowerCase().includes(searchInput.toLowerCase()) ||                     
            value.society_address.toLowerCase().includes(searchInput.toLowerCase())            
          );
        });
    ...
    exportCsv(){
      var csvRow = [];
      var newarray = [['id','Society Name','Society Number','Society Name In Regional Language','Society Address','Society Incharge Name','Society Incharge Name in Regional Language','Pin Code','Mobile Number','User ID','Dairy Name']];
      var re = this.state.filteredUsers;
      for(var item=0;item<re.length;item++){
        newarray.push([item,re[item].society_name,re[item].society_number,re[item].society_name1,re[item].society_address,re[item].rmcuinchargename,re[item].rmcuinchargename1,re[item].pin_code,re[item].mobile_number,re[item].user_id,re[item].union_name]);
      }
  ...
  <Input id="input-search" data={this.state.filteredUsers} onChange={this.handleChange} handleSetData={this.handleSetData} type = "text" placeholder = "Search Society" value={this.state.searchInput || ""} name = "search" id = "search"/>&nbsp;
              </CardHeader>
              <CardBody>
