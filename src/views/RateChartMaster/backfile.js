  ...
  return(
   ...
   {props.headers.map((item,index) => <th key={index}>{item}</th>)}
    ...
    <tbody>
     ...
     {props.formElements.map((item,index) => <tr key={index}><td>{item.ratechartid}</td><td>{item.ratechartmatrixvalue}</td><td>{item.increvalue}</td><td>{item.matrixfatrangefrom}</td><td>{item.matrixfatrangeto}</td><td>{item.condition}</td></tr> )}
    </tbody>
</Table>
    )
  }
class RateChartMaster extends Component {
    ...
    submitData(e) {
      ...
      .then(response => {
       ...
       axios.post(API_BASE_URL+'/ratechartmatrixform',{
        ...
        alert("Rate Chart Added successfully"),
    )
    ...
    const renderTodos = currentTodos.map((partners, index) => {
       const rate = JSON.parse(partners.rate_field);
      return <tbody key={partners.id}>

{rate.map(item => <tr key = {item.id}><td key ={item.id}>{partners.id}</td><td>{item.ratechartmatrixvalue}</td><td>{item.increvalue}</td><td>{item.matrixfatrangefrom}</td><td>{item.matrixfatrangeto}</td><td>{item.condition}</td></tr>)}
  ...
  <Button type="button" size="md" color="primary" onClick={() => this.AddElementsOnSubmit()}><i className="fa fa-dot-circle-o"></i> Add Condition</Button>
                ...
                <Card>
                        ...
                        <TableFormList
            headers={["Chart Id", "Field","Inc/Dec","Range From","Range To","Condition"]}
