...
class ProductList extends Component {
    ...
    addproduct(){
      let path = `/listproduct/productmaster/`;
      this.props.history.push(path);
    }
  ...
  btnNextClick() {
      ...
      globalSearch = () => {
        ...
        let filteredData = this.state.allUsers.filter(value => {
          //console.log("",value);         
          return (
            value.product_name.toLowerCase().includes(searchInput.toLowerCase())            
          );
        });
      ...
      handleClickDelete = (id) => {
          axios.post(API_BASE_URL+'/deleteproduct/' + id)
          .then(response => {
    ...
    render() {
      ...
      const renderTodos = currentTodos.map((partners, index) => {
        ...
        <td>
            {partners.product_name}
        </td>
        <td>{partners.product_name1}</td>
        <td>{partners.record_list}</td>
        <td>{partners.suppliername}</td>
        <td>{partners.unitscalename}</td>
        <td>{partners.subsidy_percentage}</td>
        <td>

        <i className="fa fa-trash fa-lg float-center" onClick={()=>this.deleteproduct(partners.id)}></i>&nbsp;&nbsp;
        <i className="fa fa-edit fa-lg float-center" onClick={()=>this.editproduct(partners.id)}></i>
     ...
     return(
        ...
        <Card>
              ...
              <CardHeader>
                ...
                <Button size="lg" onClick={()=>{this.addproduct()}}>Add Product</Button>&nbsp;
  <Input id="input-search" data={this.state.filteredUsers} onChange={this.handleChange} handleSetData={this.handleSetData} type = "text" placeholder = "Search Product" value={this.state.searchInput || ""} name = "search" id = "search"/>&nbsp;
              </CardHeader>
              <CardBody>
                <Table responsive bordered>
                  ...
                  <tr>
                    <th>Product Name</th>
                    <th> Product Name In Regional Language</th>
                    <th>Record</th>
